...
def train_func(args, model, train_batches=None, tokenizer_decoder=None, token_level=None):
    ...
    for i, idx in enumerate(range(len(train_batches))):
        ...
        outputs = model.decoder(input_ids=labels, past=z, labels=labels.long(), label_ignore=50257, is_role=False, role_label_ignore=None, role_ids=None)
        ...
        for layer in range(12):
            ...
            plt.colorbar()
            plt.savefig(f"layer{layer+1}.pdf", format="pdf", bbox_inches="tight")
            plt.show()
            exit()
...
if __name__ == '__main__':
    ...
    dic = {
        ...
        'pretrain_model_path': './checkpoints/optimus_graphenc_transformerCONV_add_syntax_Q(encode)_beta_0_latent_768_epoch_100_batch_64',
        ...
        'pt_lm':'t5-small',
        'save_dir': './checkpoints/optimus_graphenc_transformerCONV_add_syntax_Q(encode)_beta_0_latent_768_epoch_100_batch_64',
        'seed':0,
        'w2v_weights':None
    }
