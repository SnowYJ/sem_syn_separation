...
class LMFLayer(nn.Module):
    def __init__(self, rank=4, latent_size=384):
        ...
        self.expand = nn.Sequential(nn.Linear(self.hidden_size, int(self.hidden_size*2)))
    ...
    def forward(self, hidden_states, latent):
        ...
        # latent_cat = torch.ones(batch_size, seq_len, 1, dtype=torch.float, device=device)
        text_cat = torch.ones(batch_size, 1, dtype=torch.float, device=device)
...
class GPT2Model(GPT2PreTrainedModel):
    ...
    def __init__(self, config, latent_size, vocab_role_size, fuse):
        ...
        if fuse in ['uni_direction', 'bi_direction']:
            ...
            self.linear = nn.Linear(self.latent_size, config.hidden_size * config.n_layer, bias=False) # different latent vector for each layer
            self.lmf = LMFLayer(8, int(self.latent_size/2))
        else:
            self.linear_content = nn.Linear(int(self.latent_size/2), int(config.hidden_size/2) * config.n_layer, bias=False)
            self.linear_struct = nn.Linear(int(self.latent_size/2), int(config.hidden_size/2) * config.n_layer, bias=False)
        ...
        if past is None:
            ...
            if latent_as_gpt_memory:
                ...
                if fuse == 'uni_direction':
                    ...
                    past = self.linear(past)
                    # past_struct1 = self.linear_struct(past_struct).view(past_struct.shape[0], self.config.n_layer, -1)
                    # past_content1 = self.linear_content(past_content).view(past_content.shape[0], self.config.n_layer, -1)
                    # past = self.lmf(past_struct1, past_content1)
                    # past = torch.cat((_past, _past), 2)
                else:
                    past = self.linear(past)
