"""
let Bert learn structural information with the help of graph autoencoder.
"""
import argparse
import os
import json
...
import re
from construct_constituency_graph import parse_tree_string_to_list, parse_tree_to_graph
...
def evaluate(model, model_vgae, batches, role_pad=None, data_len=0, args=None, logger=None):
    # model.eval()
    # model_vgae.eval()
    final_loss = 0
    with torch.no_grad():
        for i in range(len(batches)):
            ...
            inputs_s, labels_s = batches[i][4], batches[i][5]
            # get gold latent z.
            avg_z_arr = []
            tot_loss_s, tot_rec_loss_s, tot_kl_loss_s = [], [], []
            if args.joint:
                for e in inputs_s:
                    # edge_emb_eq1 = model.vgae.generate_edge_emb(e['equation1']["node_list"])
                    # edge_index = torch.tensor(e['equation1']["edge_index"], device=inputs.device)
                    loss_s, rec_loss_s, kl_loss_s, z = model.vgae.loss_and_z(e['equation1']) # (node_num by hidden_size)
                    avg_z_arr.append(list(torch.mean(z, dim=0)))
                    tot_loss_s.append(loss_s)
                    tot_rec_loss_s.append(rec_loss_s)
                    tot_kl_loss_s.append(kl_loss_s)
            else:
                for e in inputs_s:
                    edge_emb_eq1 = model_vgae.generate_edge_emb(e['equation1']["node_list"])
                    edge_index = torch.tensor(e['equation1']["edge_index"], device=inputs.device)
                    z = model_vgae.encode(edge_emb_eq1, edge_index) # (node_num by hidden_size)
                    avg_z_arr.append(list(torch.mean(z, dim=0)))

            loss_rec, loss_kl, loss, loss_bow, loss_struct = model.autoenc(inputs.long(), labels.long(), inputs_bow, labels_bow,
                                                                           inputs_s, labels_s, args, None, infer=False, sep_infer_input=None,
                                                                           vgae_z=torch.tensor(avg_z_arr, device=inputs.device),
                                                                           tot_loss_s=torch.tensor(0, device=inputs.device, dtype=torch.float),
                                                                           tot_rec_loss_s=torch.tensor(0, device=inputs.device, dtype=torch.float),
                                                                           tot_kl_loss_s=torch.tensor(0, device=inputs.device, dtype=torch.float))
...
def train_func(args, model, model_vgae, train_batches=None, test_batches=None, logger=None):
    ...
    for epoch in range(args.epochs):
        ...
        for i, idx in enumerate(range(len(train_batches))):
            ...
            inputs_s, labels_s = train_batches[idx][4], train_batches[idx][5]
            ...
            if (i + 1) % args.log_interval == 0:
                ...
                # print('| epoch {:3d} | {:5d}/{:5d} batches | train loss {:.4f} , kl {:.4f} , latent rec {:.4f}, content {:.4f}, struct {:.4f}'.format(epoch + 1, i, len_train_dataloader, loss, loss_kl, loss_rec, loss_bow, loss_struct))
        ...
        test_loss = evaluate(model, model_vgae, test_batches, args.model, args=args, logger=logger)
...
def main(args):
    ...
    # ------------------------------------------------------------------------------------------------------------------
    args.exp = 'recon_symbol' # 'recon_natural', 'recon_symbol'
    args.joint = True # multi-task: training Optimus and VGAE jointly
    token_level = 'char_add_latex_tokens_without_var' # char, subword, char_for_latex_only, char_add_latex_tokens_without_var
    # graph model don't has include_var default remove word content.
    # ------------------------------------------------------------------------------------------------------------------
    tokenizer_encoder, tokenizer_decoder = load_optimus_tokenizer(logger, token_level=token_level)
    if args.exp == 'inference':
        print('inference task')
        train = MathInferenceCorpus(args.train_corpus)
        test = MathInferenceCorpus(args.test_corpus)
    elif args.exp in ['recon_natural', 'recon_symbol']:
    ...
    def tokenize_function(examples, is_symbol=args.exp != 'recon_natural'):
        if is_symbol:
    ...
    if args.exp == 'recon_natural':
        # building Vocab for RNN and vgae model.
    ...
    if args.joint:
        ...
        print("loading pretrained VGAE")
        checkpoint = torch.load("checkpoints/VGAE/training.bin", map_location=torch.device('cpu'))
        model_vgae = VGraphAE(device, input_dim=384, hidden_dim=384)
        model_vgae.load_state_dict(checkpoint, strict=True)
    # ------------------------------------------------------------------------------------------------------------------------------

    train_par = {'data': train_sents, 'data_bow': train_sents_bow, 'data_struct': train_sents_s, 'batch_size': args.batch_size,
                 'vocab': vocab, 'vocab_s': vocab_s, 'device': device, 'model_type': args.model_type}
    train_batches, _ = get_batches(**train_par)

    val_par = {'data': valid_sents, 'data_bow': valid_sents_bow, 'data_struct': valid_sents_s, 'batch_size': args.batch_size,
               'vocab': vocab, 'vocab_s': vocab_s, 'device': device, 'model_type': args.model_type}
    valid_batches, _ = get_batches(**val_par)
    # ------------------------------------------------------------------------------------------------------------------------------

    train_func(args, model, model_vgae, train_batches=train_batches, test_batches=valid_batches, logger=logger)
...
if __name__ == '__main__':
    ...
    dic = {
        ...
        # 'train_corpus':'./math_symbolic_dataset/recon_graph/both_tr.txt',
        # 'test_corpus': './math_symbolic_dataset/recon_graph/both_te.txt',
