...
from optimus_separate_graph.data_access.math_symbolic import MathInferenceCorpus, MathReconstructCorpus, get_batches, conv_sent_dict
from optimus_separate_graph.examples.big_ae.utils import frange_cycle_zero_linear
from optimus_separate_graph.examples.big_ae.modules import OptimusVAE, VGraphAE
from python_transformers.optimization import AdamW, get_linear_schedule_with_warmup
from optimus_separate_graph.pytorch_transformers import (AdamW, BertConfig, BertForLatentConnector, BertTokenizer, GPT2Config, GPT2ForLatentConnector, GPT2Tokenizer)
import torch
...
def load_graph_optimus(args, logger, graph_encoder, tokenizer_decoder, tokenizer_encoder):
    ...
    model_decoder = decoder_model_class.from_pretrained('gpt2', cache_dir=None, config=decoder_config, latent_size=args.latent_size, latent_as_gpt_emb=False, latent_as_gpt_memory=True)
...
def main(args):
    ...
    model_vgae = VGraphAE(device, input_dim=int(args.latent_size/2), hidden_dim=int(args.latent_size/2), num_layers=2, sym_dict=vocab_s, heads=8, include_var=include_var, graph_type=graph_type)
    model = load_graph_optimus(args, logger, graph_encoder=model_vgae, tokenizer_decoder=tokenizer_decoder, tokenizer_encoder=tokenizer_encoder)
    ...
    val_par = {'data': valid_sents, 'data_struct': valid_sents_s,'batch_size': args.batch_size, 'model': model, 'device': device, 'include_var': include_var}
