...
class Attention(nn.Module):
    def __init__(self, nx, n_ctx, config, scale=False, fuse=None):
        ...
        elif fuse == 'fuse_syn_Q_sem_KV':
            self.lmf_q = LMFLayer(4, 768, 768)
            self.lmf_kv = LMFLayer(4, 768, 768)
        elif fuse in ['add_syntax_query', 'add_syn_Q_sem_KV']:
            pass
        else:
    ...
    def forward(self, x, layer_past=None, attention_mask=None, head_mask=None, latent_syntax=None):
        ...
        elif self.fuse == 'add_syntax_query':
            query = query + latent_syntax
        elif self.fuse == 'add_syn_Q_sem_KV':
            query = query + latent_syntax
            key = key + layer_past[0]
            value = value + layer_past[1]
        ...
        elif self.fuse == 'fuse_syn_Q_sem_KV':
            past_key, past_value = layer_past[0], layer_past[1]
            query = self.lmf_q(query, latent_syntax.squeeze(1))
            key = self.lmf_kv(key, past_key.squeeze(1))
            value = self.lmf_kv(value, past_value.squeeze(1))
        elif self.fuse == 'fuse_syntax_query_key_value_one_fuse_model':
            query = self.lmf(query, latent_syntax.squeeze(1))
            key = self.lmf(key, latent_syntax.squeeze(1))
            value = self.lmf(value, latent_syntax.squeeze(1))
        elif self.fuse in ['fuse_syntax_query_key_value_sep_head', 'fuse_syntax_query_key_value_sep_head_one_fuse_model', 'fuse_syntax_query_key_value_sep_head_no_mask', 'fuse_syntax_query_key_value_sep_head_one_fuse_model_no_mask']:
            #  head sep
        ...
        if layer_past is not None:
            ...
            if self.fuse in ['fuse_syn_Q_sem_KV', 'add_syn_Q_sem_KV']:
                pass
            else:
        ...
        attn_outputs = self._attn(query, key, value, attention_mask, head_mask) # 1, 12, 10, 64
        """ 
        ...
        """
        a shape: torch.Size([batch_size, seq_length, 768])
        present: ([2: (KV), 1: (batch_size), 12: (head), 11: (seq_length + 1), 64])
        """
...
class Block(nn.Module):
    ...
    def forward(self, x, layer_past=None, attention_mask=None, head_mask=None, latent_syntax=None):
        ...
        # print(len(output_attn))
        # print(output_attn[-1].shape)
        return outputs  # x, present, (attentions) (1, 12(head), 11(length), 64)
...
class GPT2Model(GPT2PreTrainedModel):
    ...
    def __init__(self, config, latent_size, vocab_role_size, fuse):
        ...
        if fuse in ['uni_direction', 'bi_direction']:
            ...
            self.linear = nn.Linear(self.latent_size, config.hidden_size * config.n_layer, bias=False) # different latent vector for each layer
        elif fuse in ['fuse_syntax_query', 'fuse_syntax_key', 'fuse_syntax_value', 'fuse_syn_Q_sem_KV', 'add_syntax_query', 'add_syn_Q_sem_KV']:
