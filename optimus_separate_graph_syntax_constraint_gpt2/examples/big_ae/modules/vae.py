...
class OptimusVAE(nn.Module):
    ...
    """
    def __init__(self, language_encoder, graph_encoder, decoder, tokenizer_decoder, args): #
        ...
        # self.encoder_linear = nn.Linear(args.latent_size, args.latent_size*2)
        self.language_linear = nn.Linear(int(args.latent_size/2), args.latent_size, bias=False)
        self.graph_linear = nn.Linear(int(args.latent_size/2), args.latent_size, bias=False)

    def connect(self, pooled_hidden_fea_language, pooled_hidden_fea_graph, nsamples=1, output_mu_sig=False):
        """
        Returns: Tensor1, Tensor2
            Tensor1: the tensor latent z with shape [batch, nsamples, nz]
            Tensor2: the tenor of KL for each x with shape [batch]
        """
        mean_lm, logvar_lm = self.language_linear(pooled_hidden_fea_language).chunk(2, -1)
        mean_s, logvar_s = self.graph_linear(pooled_hidden_fea_graph).chunk(2, -1)

        if output_mu_sig:
            return mean_lm, logvar_lm, mean_s, logvar_s

        z_lm = self.reparameterize(mean_lm, logvar_lm, nsamples)
        z_s = self.reparameterize(mean_s, logvar_s, nsamples)

        KL_lm = 0.5 * (mean_lm.pow(2) + logvar_lm.exp() - logvar_lm - 1).sum(dim=1)
        KL_s = 0.5 * (mean_s.pow(2) + logvar_s.exp() - logvar_s - 1).sum(dim=1)

        z = torch.cat((z_lm, z_s), dim=2)
        KL = KL_lm + KL_s
    ...
    def get_concat_latent(self, inputs_lm, inputs_graph, output_mu_sig=False):
        """
        get concat latent z of disentangled Optimus
        """
        attention_mask = (inputs_lm > 0).float()
        pooled_hidden_fea_lm = self.language_encoder(inputs_lm, attention_mask, is_role=False, role_ids=None)[1]

        z, _, graph_rec_loss, _ = self.graph_encoder(inputs_graph.x.to(inputs_lm.device), inputs_graph.edge_index.to(inputs_lm.device))
        ...
        batch_assignment = inputs_graph.batch  # Shape: (total_num_nodes,)
        ...
        pooled_hidden_fea_graph = torch.stack(individual_graphs).to(inputs_lm.device)
        # pooled_hidden_fea = torch.cat((pooled_hidden_fea_lm, pooled_hidden_fea_graph), dim=1)

        if output_mu_sig:
            return self.connect(pooled_hidden_fea_lm, pooled_hidden_fea_graph, output_mu_sig=output_mu_sig)

        latent_z, loss_kl = self.connect(pooled_hidden_fea_lm, pooled_hidden_fea_graph)
        latent_z = latent_z.squeeze(1)
        return latent_z

    def autoenc(self, inputs_lm, inputs_s, labels):
        w_loss_rec, w_loss_kl, w_loss, loss_rec_graph = self(inputs_lm=inputs_lm, inputs_graph=inputs_s, labels=labels)
    ...
    def loss_rec_struct(self, logits, targets):
        loss = F.cross_entropy(logits.view(-1, logits.size(-1)), targets.view(-1), ignore_index=self.vocab_s.pad, reduction='none').view(targets.size())
        return loss.mean(dim=0)

    def forward(self, inputs_lm, inputs_graph, labels):
        reconstrution_mask = (labels != 50257).float()
        attention_mask = (inputs_lm > 0).float()
        sent_length = torch.sum(reconstrution_mask, dim=1)

        z, _, graph_rec_loss, _ = self.graph_encoder(inputs_graph.x.to(labels.device), inputs_graph.edge_index.to(labels.device))
        ...
        pooled_hidden_fea_graph = torch.stack(individual_graphs).to(labels.device)
        pooled_hidden_fea_language = self.language_encoder(inputs_lm, attention_mask, is_role=False, role_ids=None)[1]
        # pooled_hidden_fea = self.concat_linear(torch.cat((pooled_hidden_fea_language, pooled_hidden_fea_graph), dim=1))

        if self.args.fb_mode == 0:
            latent_z, loss_kl = self.connect(pooled_hidden_fea_language, pooled_hidden_fea_graph)
            latent_z = latent_z.squeeze(1)
            ...
            mu, logvar = self.language_linear(pooled_hidden_fea_language).chunk(2, -1)
            z_lm = self.reparameterize(mu, logvar, nsamples=1)
            loss_kl = 0.5 * (mu.pow(2) + logvar.exp() - logvar - 1)
            kl_mask = (loss_kl > self.args.dim_target_kl).float()
            loss_kl_lm = (kl_mask * loss_kl).sum(dim=1)

            mu, logvar = self.graph_linear(pooled_hidden_fea_graph).chunk(2, -1)
            z_s = self.reparameterize(mu, logvar, nsamples=1)
            loss_kl = 0.5 * (mu.pow(2) + logvar.exp() - logvar - 1)
            kl_mask = (loss_kl > self.args.dim_target_kl).float()
            loss_kl_s = (kl_mask * loss_kl).sum(dim=1)

            latent_z = torch.cat((z_lm, z_s), dim=2).squeeze(1)
            loss_kl = loss_kl_lm + loss_kl_s
            # Decoding
