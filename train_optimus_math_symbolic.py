...
# from optimus.utils import logging, conv_sent_dict
from optimus.data_access.math_symbolic import MathInferenceCorpus, MathReconstructCorpus, get_batches, conv_sent_dict
from optimus.examples.big_ae.utils import frange_cycle_zero_linear
...
from optimus.pytorch_transformers import (AdamW, BertConfig, BertForLatentConnector, BertTokenizer, GPT2Config, GPT2ForLatentConnector, GPT2Tokenizer)
import torch
MODEL_CLASSES={
...
def load_optimus(args, logger, token_level='char'):
...
def evaluate(model, batches, role_pad=None, data_len=0, args=None, logger=None):
    ...
    with torch.no_grad():
        for i in range(len(batches)):
            ...
            inputs_1 = None
            loss_rec, loss_kl, loss = model.autoenc(inputs.long(), labels.long(), input_roles, label_roles, args, role_pad, data_len, infer=False, sep_infer_input=inputs_1)
...
def train_func(args, model, train_batches=None, test_batches=None, logger=None):
    ...
    for epoch in range(args.epochs):
        ...
        for i, idx in enumerate(range(len(train_batches))):
            ...
            loss_rec, loss_kl, loss = model.autoenc(inputs.long(), labels.long(), input_roles, label_roles, args, None, infer=False, sep_infer_input=inputs_1)
...
def main(args):
    ...
    # ------------------------
    exp = 'recon' # recon_natural, recon_symbol, inference
    token_level = 'subword' # 'char_add_latex_tokens_without_var'
    # subword (default tokenization), char (char-level tokenization), char_for_latex_only, subword_add_latex_tokens, char_add_latex_tokens_without_var
    include_var = False
    ...
    if exp == 'inference':
        ...
        train = MathReconstructCorpus(args.train_corpus, type=type) # content, struct, content_struct
        test = MathReconstructCorpus(args.test_corpus, type=type) # content, struct, content_struct

    conv_sent_dict_param = {'emb_tokenizer': tokenizer_encoder, 'decode_tokenizer': tokenizer_decoder, 'token_level': token_level,
                            'include_var': include_var, 'type': type}
    ...
    train_par = {'data': train_sents, 'batch_size': args.batch_size, 'device': device}
...
if __name__ == '__main__':
    ...
    dic = {
        ...
        # 'train_corpus':'./math_symbolic_dataset/encoder_inference/both_tr.txt',
