...
class OptimusVAE(nn.Module):
    ...
    def __init__(self, encoder, decoder,  tokenizer_encoder, tokenizer_decoder, args, vocab): #
        ...
        # Struct
        print('using siamese for struct space')
    ...
    def connect(self, bert_fea, bert_fea_s, nsamples=1):
        """
        Returns: Tensor1, Tensor2
            Tensor1: the tensor latent z with shape [batch, nsamples, nz]
            Tensor2: the tenor of KL for each x with shape [batch]
        ...
        mean, logvar = self.encoder.linear_structure(bert_fea_s).chunk(2, -1)
    ...
    def get_concat_latent(self, inputs, inputs_s):
        """
        ...
        attention_mask = (inputs_s > 0).float()
        outputs = self.encoder(inputs_s, attention_mask, is_role=False, role_ids=None)
        pooled_hidden_fea_s = outputs[1]

        latent_z_content, loss_kl_content, latent_z_struct, loss_kl_struct = self.connect(pooled_hidden_fea, pooled_hidden_fea_s)
    ...
    def autoenc(self, inputs, labels, inputs_bow, labels_bow, inputs_s, labels_s, args):
        if args.model == 'optimus':
            w_loss_rec, w_loss_kl, w_loss, loss_bow, loss_struct = self(inputs=inputs.long(), labels=labels, inputs_bow=inputs_bow,
                                                                        labels_bow=labels_bow, inputs_struct=inputs_s, labels_struct=labels_s)
            w_loss_rec, w_loss_kl, w_loss = w_loss_rec.mean(), w_loss_kl.mean(), w_loss.mean()
            return (w_loss_rec, w_loss_kl, w_loss, loss_bow.detach(), loss_struct)
        else:
            exit('ERROR: optimus training failure in autenc() func')

    def forward(self, inputs, labels, inputs_bow, labels_bow, inputs_struct, labels_struct):
        ...
        # latex
        pooled_hidden_fea = self.encoder(inputs, attention_mask, is_role=False, role_ids=None)[1]

        # structure
        attention_mask = (inputs_struct > 0).float()
        pooled_hidden_fea_s = self.encoder(inputs_struct, attention_mask, is_role=False, role_ids=None)[1]
        ...
        if self.args.fb_mode == 0:
            ...
            # loss_bow = nn.BCELoss()(content_preds.to(inputs.device), labels_bow.type(torch.FloatTensor).to(inputs.device))
            loss_bow = torch.tensor(0, device=inputs.device)
            loss_struct = torch.tensor(0, device=inputs.device)
