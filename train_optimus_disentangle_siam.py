...
from optimus_disentangle_siam.data_access.math_symbolic import MathInferenceCorpus, MathReconstructCorpus, get_batches, conv_sent_dict, conv_sent_dict_s
from optimus_disentangle_siam.examples.big_ae.utils import frange_cycle_zero_linear
...
from optimus_disentangle_siam.pytorch_transformers import (AdamW, BertConfig, BertForLatentConnector, BertTokenizer, GPT2Config, GPT2ForLatentConnector, GPT2Tokenizer)
import torch
...
def evaluate(model, batches, args=None, logger=None):
    ...
    with torch.no_grad():
        for i in range(len(batches)):
            ...
            loss_rec, loss_kl, loss, loss_bow, loss_struct = model.autoenc(inputs.long(), labels.long(), inputs_bow, labels_bow, inputs_s, labels_s, args)
...
def main(args):
    ...
    elif exp == 'recon_symbol':
        print('math symbol reconstruction task')
        train = MathReconstructCorpus(args.train_corpus)
        test = MathReconstructCorpus(args.test_corpus)
    elif exp == 'recon_natural':
        print('natural language reconstruction task')
        train = MathReconstructCorpus(args.train_corpus)
        test = MathReconstructCorpus(args.test_corpus)
    else:
    ...
    for sent in tqdm(train):
        ...
        tr_temp = conv_sent_dict_s(sent, **conv_sent_dict_param)
        train_sents_s.append(tr_temp)
        # ----------------------
        # Content
    ...
    for sent in tqdm(test):
        ...
        val_temp = conv_sent_dict_s(sent, **conv_sent_dict_param)
        valid_sents_s.append(val_temp)
        # ----------------------
        # Content
        p, c = sent['premises'], sent['conclusion']
        if token_level == 'char_add_latex_tokens_without_var':
    ...
    train_par = {'data': train_sents, 'data_bow': train_sents_bow, 'data_struct': train_sents_s, 'batch_size': args.batch_size, 'vocab': vocab, 'device': device}
