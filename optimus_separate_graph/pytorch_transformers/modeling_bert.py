...
class BertForLatentConnector(BertPreTrainedModel):
    ...
    def __init__(self, config, latent_size, vocab_role_size=1):
        ...
        # self.linear = nn.Linear(config.hidden_size, 2 * latent_size, bias=False) # learning latent embedding
        #
        # self.linear_content = nn.Linear(config.hidden_size, latent_size, bias=False)
        # self.linear_structure = nn.Linear(config.hidden_size, latent_size, bias=False)
        self.pool_linear = nn.Linear(config.hidden_size, latent_size, bias=False)
    ...
    def forward(self, input_ids, attention_mask=None, token_type_ids=None, position_ids=None, head_mask=None, is_role=False, role_ids=None):
        ...
        sequence_output = encoder_outputs[0]
        pooled_output = self.pool_linear(self.pooler(sequence_output))
